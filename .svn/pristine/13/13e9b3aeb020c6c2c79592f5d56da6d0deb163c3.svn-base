 ...
 * @version 创建时间：2015-11-27 上午10:48:56
 * @function ： 实用的3d效果广告栏切换用的viewpager 与普通的没什么区别，只是集成时用的
 */
public class ViewPager_3DBanner extends ViewPager {
    ...
    public ViewPager_3DBanner(Context context) {
	this(context, null);
    }

    @SuppressWarnings("incomplete-switch")
    public ViewPager_3DBanner(Context context, AttributeSet attrs) {
	super(context, attrs);
	...
	for (int i = 0; i < getChildCount(); i++) {
	    View v = getChildAt(i);
	    if (!(v instanceof FrameLayout_3DBanner)) {
		removeView(v);
	...
	if (!mOutlineEnabled || child instanceof FrameLayout_3DBanner)
	    return child;
	FrameLayout_3DBanner out = new FrameLayout_3DBanner(getContext());
	out.setLayoutParams(generateDefaultLayoutParams());
	child.setLayoutParams(new FrameLayout_3DBanner.LayoutParams(
	...
	if (!(left instanceof FrameLayout_3DBanner))
	    return;
	if (mState != State.IDLE) {
	    if (left != null) {
		((FrameLayout_3DBanner) left).setOutlineAlpha(1.0f);
	    }
	    if (right != null) {
		((FrameLayout_3DBanner) right).setOutlineAlpha(1.0f);
	    }
	} else {
	    if (left != null)
		((FrameLayout_3DBanner) left).start();
	    if (right != null)
		((FrameLayout_3DBanner) right).start();
	}
    }
